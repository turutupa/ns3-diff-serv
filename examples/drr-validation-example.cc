...
int main(int argc, char *argv[]) {
  ...
  LogComponentEnable("DrrValidation", LOG_LEVEL_INFO);
  ...
  p2p.SetDeviceAttribute("DataRate", DataRateValue(DataRate(100000)));
  ...
  UdpServerHelper midServer(midPort);
  apps = midServer.Install(n2);
  apps.Start(Seconds(serverListenStartTime));
  apps.Stop(Seconds(stopTime));
  ...
  apps.Start(Seconds(clientStartTime));
  apps.Stop(Seconds(clientStopTime));
  // Mid priority port and install
  client.SetAttribute("RemotePort", UintegerValue(midPort));
  apps = client.Install(n0);
  apps.Start(Seconds(clientStartTime));
  apps.Stop(Seconds(clientStopTime));
  // Low priority port and install
